...
async def start(client, message):
    if message.chat.type in ['group', 'supergroup']:
        ...
        if not await db.get_chat(message.chat.id):
            ...
            await client.send_message(LOG_CHANNEL, script.LOG_TEXT_G.format(message.chat.title, message.chat.id, message.chat.link, total, "Unknown"))       
            await db.add_chat(message.chat.id, message.chat.title, message.chat.link)
        return await message.reply(script.START_TXT.format(message.from_user.mention if message.from_user else message.chat.title else message.chat.link), reply_markup=reply_markup)
