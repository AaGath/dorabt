...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer(f"Hey {query.from_user.first_name} This Not For You Request Your Own Movie/Series", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
സിനിമ ലഭിക്കണം എങ്കിൽ താങ്കൾ ഗുഗിൾ നോക്കി സിനിമയുടെ correct spelling ഇവിടെ send ചെയ്യുക എങ്കിലേ താങ്കൾ ഉദ്ദേശിക്കുന്ന സിനിമ എനിക്ക് അയച്ചു തരാൻ കഴിയുകയുളളു 😀""",show_alert=True)
        ...
        buttons= [[
            InlineKeyboardButton('🤖 Updates', url='https://t.me/NewMoviesPF'),
